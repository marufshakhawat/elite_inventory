    ...
    <div className="space-y-16 pb-20">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 w-full text-white">
          <div className="max-w-3xl space-y-8 animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <Link to="/shop" className="group relative inline-flex items-center justify-center px-10 py-5 rounded-full font-semibold text-slate-900 bg-white hover:bg-slate-100 transition-all text-sm uppercase tracking-widest shadow-2xl">
                Start Exploring <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/about" className="inline-flex items-center justify-center px-10 py-5 rounded-full font-semibold text-white bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 transition-all text-sm uppercase tracking-widest">
                Our Guarantee
              </Link>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, idx) => (
            <div key={idx} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 flex flex-col items-center text-center space-y-2 hover:translate-y-[-5px] transition-transform">
              ...
              <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-widest">{stat.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="text-center mb-12 space-y-2">
          <h2 className="text-3xl font-semibold text-slate-900 tracking-tight">Browse by Category</h2>
          <p className="text-slate-400 text-sm font-medium">Find the specific tools your workflow demands.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, idx) => (
            ...
            >
              ...
              <h3 className="text-xl font-semibold text-slate-900 tracking-tight">{cat.name}</h3>
              <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-widest mt-1">{cat.count}</p>
              <div className="mt-6 flex items-center gap-2 text-slate-900 font-semibold text-[10px] uppercase tracking-widest group-hover:gap-4 transition-all">
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="flex justify-between items-end mb-10">
          <div className="space-y-1">
            <h2 className="text-3xl font-semibold text-slate-900 tracking-tight">Trending Assets</h2>
            <p className="text-sm text-slate-400 font-medium">The most requested professional subscriptions right now.</p>
          </div>
          <Link to="/shop" className="group flex items-center gap-2 text-xs font-semibold text-slate-900 uppercase tracking-widest border-b-2 border-slate-900 pb-1 hover:opacity-70 transition-opacity">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-1 rounded-full">
                ...
                <span className="text-[9px] font-semibold uppercase tracking-widest text-slate-400">Elite Verification Protocol</span>
              </div>
              <h2 className="text-4xl md:text-5xl font-semibold leading-tight">Authenticity in every pixel.</h2>
              ...
              <div className="flex flex-wrap gap-6 pt-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-widest text-slate-300">Official Licenses</span>
                </div>
                <div className="flex items-center gap-3">
                  <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                  <span className="text-xs font-semibold uppercase tracking-widest text-slate-300">Secure Delivery</span>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/5 backdrop-blur-md p-8 rounded-[2rem] border border-white/10 text-center">
                ...
                <h4 className="font-semibold text-xl">Elite Tier</h4>
                <p className="text-[9px] text-slate-500 uppercase font-semibold tracking-widest mt-1">Vendor Status</p>
              </div>
              <div className="bg-white/5 backdrop-blur-md p-8 rounded-[2rem] border border-white/10 text-center translate-y-8">
                ...
                <h4 className="font-semibold text-xl">Verified</h4>
                <p className="text-[9px] text-slate-500 uppercase font-semibold tracking-widest mt-1">Key Integrity</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="text-center mb-16 space-y-2">
          <h2 className="text-3xl font-semibold text-slate-900 tracking-tight">Operative Feedback</h2>
          <p className="text-slate-400 text-sm font-medium">Real stories from our professional community.</p>
        </div>
          ...
          {[
            ...
            <div key={idx} className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm relative group hover:shadow-xl transition-all">
              ...
              <div className="mt-8 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-slate-900 flex items-center justify-center text-white font-semibold text-sm uppercase">
                  {item.name.split(' ').map(n => n[0]).join('')}
                </div>
                <div>
                  <h4 className="font-semibold text-slate-900 text-sm">{item.name}</h4>
                  <p className="text-[9px] text-slate-400 font-semibold uppercase tracking-widest">{item.role}</p>
                </div>
              </div>
            </div>
          ))}
